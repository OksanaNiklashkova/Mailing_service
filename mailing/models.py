...
class Recipient(models.Model):
    ...
    comment = models.TextField(
        max_length=150, default="", blank=True, null=True, verbose_name="Комментарий"
    )
...
class Message(models.Model):
    ...
    text = models.TextField(
        default="", blank=True, null=True, verbose_name="Текст сообщения"
    )
    owner = models.ForeignKey(
        CustomUser,
        on_delete=models.CASCADE,
...
class SendAttempt(models.Model):
    ...
    attempt_datetime = models.DateTimeField(
        default=timezone.now, verbose_name="Дата попытки"
    )
    status = models.CharField(
        max_length=10,
        choices=STATUS_CHOICES,
        default=STATUS_UNSUCCES,
    ...
    def __str__(self):
        return f"Попытка для рассылки {self.mailing} от пользователя {self.owner} — {self.status}\
         ({self.attempt_datetime})"
